...
static_assert(
    sizeof(StateInfo) == 2 * sizeof(Cost) + sizeof(void *),
...
class ShortestPaths {
    ...
    utils::LogProxy &log;
    const int max_cached_shortest_paths;
    bool use_cache;
    const bool debug;
    ...
    // Parents are optimal outgoing transitions and children are optimal
    // outgoing transitions from some state pointing to the current state.
    // `reverse_parents` and `reverse_children` are the same for incoming
    // transitions, which is needed for regression flaws.
    std::deque<Transitions> parents;
    std::deque<Transitions> children;
    std::deque<Transitions> reverse_parents;
    std::deque<Transitions> reverse_children;
    int num_cached_shortest_paths;

    // Store single shortest path for each state if use_cache=false.
    std::deque<Transition> parent;
    std::deque<Transition> reverse_parent;
    ...
    void update_incrementally_in_direction(
        const Abstraction &abstraction,
        int v, int v1, int v2,
        const std::optional<Transitions> &old_incoming,
        const std::optional<Transitions> &old_outgoing,
        int var,
        bool backward);

    void resize(int num_states);
    void set_parent(int state, const Transition &new_parent, bool reverse = false);
    void add_parent(int state, const Transition &new_parent, bool reverse = false);
    void remove_parent(int state, const Transition &parent, bool reverse = false);
    void remove_child(int state, const Transition &child, bool reverse = false);
    void clear_parents(int state, bool reverse = false);
    // void set_child(int state, const Transition &new_child);
    // void add_child(int state, const Transition &new_child);
    // void clear_children(int state);
    void mark_dirty(int state, bool reverse = false);
    ...
    bool is_optimal_backward_transition(int start_id, int op_id, int target_id) const;
...
public:
    ...
    OptimalTransitions get_optimal_backward_transitions(
        const Abstraction &abstraction, int state) const;
...
extern std::vector<int> compute_init_distances(
    const Abstraction &abstraction,
    const std::vector<int> &costs,
    const int init_id);
}
