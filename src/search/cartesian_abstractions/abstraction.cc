...
const vector<unordered_set<int>> &Abstraction::get_postcondition_set(int op_id) const {
    return transition_rewirer->get_postcondition_set(op_id);
}
...
const vector<Facts> &Abstraction::get_postconditions() const {
...
int Abstraction::get_postcondition_value(int op_id, int var) const {
    return transition_rewirer->get_postcondition_value(op_id, var);
}
...
const optional<Transitions> &Abstraction::get_old_incoming_transitions() const {
    return old_incoming;
}
const optional<Transitions> &Abstraction::get_old_outgoing_transitions() const {
    return old_outgoing;
}
...
void Abstraction::initialize_trivial_abstraction(const vector<int> &domain_sizes) {
    ...
    unique_ptr<AbstractState> init_state =
        AbstractState::get_trivial_abstract_state(CartesianSet(domain_sizes));
    init_id = init_state->get_id();
    goals.insert(init_state->get_id());
    states.push_back(move(init_state));
}

tuple<int, int> Abstraction::refine(
    ...
    unique_ptr<AbstractState> v1 = make_unique<AbstractState>(
        v1_id, node_ids.first, move(*this->cartesian_sets[node_ids.first]));
    unique_ptr<AbstractState> v2 = make_unique<AbstractState>(
        v2_id, node_ids.second, move(*this->cartesian_sets[node_ids.second]));
    ...
    if (transition_system) {
        auto [old_in, old_out] = transition_system->rewire(states, v_id, *v1, *v2, var);
        old_incoming = move(old_in);
        old_outgoing = move(old_out);
    }
