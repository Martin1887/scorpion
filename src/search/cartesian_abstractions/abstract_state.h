...
class AbstractState {
    ...
    std::vector<bool> get_possibly_triggered_effect_in_variable(const OperatorProxy &op,
                                                                const Abstraction &abs,
                                                                const std::optional<std::reference_wrapper<const CartesianSet>> &target = std::nullopt) const;
