...
void OspSymbolicUniformCostSearch::initialize_utilitiy_function() {
    ...
    if (!use_add) {
        ...
        while (cur_add != vars->get_manager()->constant(
            ...
            bdd_utility_functions[(int)min_value] =
                cur_add.BddInterval(min_value, min_value);
            cur_add = cur_add.Maximum(bdd_utility_functions[(int)min_value].Add() *
...
void OspSymbolicUniformCostSearch::add_options_to_feature(plugins::Feature &feature) {
    feature.add_option<bool>("use_add", "use utility function decomposition", "false");
}
} // namespace symbolic

class OspSymbolicUniformCostSearchFeature : public plugins::TypedFeature<SearchAlgorithm, symbolic::OspSymbolicUniformCostSearch> {
public:
    OspSymbolicUniformCostSearchFeature() : TypedFeature("OspSymbolicUniformCostSearch") {
        document_synopsis("Symbolic Forward Osp Search");
        symbolic::OspSymbolicUniformCostSearch::add_options_to_feature(*this);
        add_option<std::shared_ptr<symbolic::PlanSelector>> (
            "plan_selection", "plan selection strategy", "top_k(num_plans=1)");
        add_option<bool>("fw", "Search in the forward direction", "true");
        add_option<bool>("bw", "Search in the backward direction", "false");
    }
};

static plugins::FeaturePlugin<OspSymbolicUniformCostSearchFeature> _plugin;
