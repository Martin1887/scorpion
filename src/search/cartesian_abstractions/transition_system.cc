...
TransitionSystem::TransitionSystem(TransitionRewirer &rewirer)
    : rewirer(rewirer),
      num_non_loops(0),
      num_loops(0) {
...
pair<Transitions, Transitions> TransitionSystem::rewire(
    ...
    auto [old_incoming, old_outgoing] = rewirer.rewire_transitions(incoming, outgoing, states, v_id, v1, v2, var);
    int v1_id = v1.get_id();
    int v2_id = v2.get_id();
    num_non_loops +=
...
const vector<unordered_set<int>> &TransitionSystem::get_postcondition_set(int op_id) const {
    return rewirer.get_postcondition_set(op_id);
}
