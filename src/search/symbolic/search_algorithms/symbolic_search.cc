...
SymbolicSearch::SymbolicSearch(const plugins::Options &opts,
    ...
    : SymbolicSearch(opts, make_shared < SymVariables > (opts, task), SymParamsMgr(opts, task), task, true) {
}

SymbolicSearch::SymbolicSearch(const plugins::Options &opts, shared_ptr < SymVariables > vars, SymParamsMgr mgrParams,
                               const shared_ptr < AbstractTask > &task, bool initialize_symbolic_vars)
    : SearchAlgorithm(opts, task),
      task(task),
      search_task(task),
      task_proxy(*task),
      vars(vars),
      ...
      min_g(0),
      plan_data_base(opts.get < std::shared_ptr < PlanSelector >> ("plan_selection")),
...
void SymbolicSearch::initialize() {
    ...
    if (simple) {
        ...
        upper_bound = min(upper_bound, (int)max_plan_cost + 1);
        cout << endl;
    }
...
void SymbolicSearch::add_options_to_feature(plugins::Feature &feature) {
    ...
    SymParamsSearch::add_options_to_feature(feature);
    SymParamsMgr::add_options_to_feature(feature);
    PlanSelector::add_options_to_feature(feature);
    feature.add_option < bool > ("silent", "silent mode that avoids writing the cost bounds",
                                 "false");
    feature.add_option < bool > ("simple", "simple/loopless plan construction",
                                 "false");
    feature.add_option < bool > ("single_solution", "search for a single solution",
                                 "true");
}
} // namespace symbolic
